#!/usr/bin/env python3
"""Score the manually annotated [M1-08] sample and publish the results.

Session 2 of a two-session task -- see ``scripts/sample_parsing_quality.py``
for session 1 (drawing the stratified sample) and its module docstring for
...
def render_markdown_report(
    ...
    lines = [
        ...
        "",
        "Generated by `scripts/score_parsing_quality.py` [M1-08] from the manually "
        "annotated 50-clause sample in `eval/parsing_quality_sample.csv` (drawn by "
        "`scripts/sample_parsing_quality.py`, stratified across product line, "
        "extraction mode and filing-year era). This is the evaluation data behind "
        "`docs/PARSING.md`, not just a table in that document -- every number here "
        "is reproducible by re-running `make score-parsing-quality` against the "
        "same annotated CSV.",
        "",
        "## Overall accuracy",
        "",
