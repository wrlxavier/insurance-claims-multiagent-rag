"""The clarification loop as a compiled graph ([M4-03]).

``route_after_intake`` in isolation, the graph's structure, and -- the DoD's
core check -- that the ``intake ⇄ clarification`` loop terminates for every
incomplete claim in the M2-04 set. Termination is a property of the router
plus ``MAX_CLARIFICATION_ROUNDS``, so a fake LLM is the right tool: no network,
and no reliance on LangGraph's ``recursion_limit``.
"""
...
def test_graph_has_the_expected_nodes_and_entry() -> None:
    ...
    assert {"intake", "clarification", "clarification_exhausted"} <= set(graph.nodes)
...
def test_complete_claim_passes_straight_through() -> None:
    ...
    assert [e.node for e in out["audit_trail"]] == ["intake"]
